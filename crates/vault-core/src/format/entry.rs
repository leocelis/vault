...
//! (constraint C11). At this layer a Protected value holds the bytes *as stored in the payload*;
//! the inner-stream ChaCha20 pass that protects them in RAM and at rest (constraint C19) is applied
//! by the open/save flow around this codec — this module only frames the structure.
...
impl Entry {
    ...
    pub fn serialize(&self) -> Vec<u8> {
        ...
        tlv::write_record(&mut out, tag::PASSWORD, self.password.expose());
        ...
        if let Some(otp) = &self.otp_secret {
            tlv::write_record(&mut out, tag::OTP_SECRET, otp.expose());
        }
        ...
        for cf in &self.custom_fields {
            ...
            match &cf.value {
                ...
                CustomValue::Protected(p) => {
                    tlv::write_record(&mut out, tag::CUSTOM_VALUE_PROTECTED, p.expose())
                }
            }
        }
        out
    ...
    pub fn parse(bytes: &[u8]) -> Result<Entry> {
        ...
        while let Some((t, v)) = tlv::read_record(&mut cur, MAX_FIELD_LEN)? {
            ...
            match t {
                ...
                tag::PASSWORD => e.password = Protected::new(v.to_vec()),
                tag::URL => e.url = tlv::decode_str(v)?,
                tag::NOTES => e.notes = tlv::decode_str(v)?,
                tag::TAG => e.tags.push(tlv::decode_str(v)?),
                tag::OTP_SECRET => e.otp_secret = Some(Protected::new(v.to_vec())),
                ...
                tag::CUSTOM_VALUE_PROTECTED => {
                    ...
                    e.custom_fields.push(CustomField {
                        name,
                        value: CustomValue::Protected(Protected::new(v.to_vec())),
                    });
                }
...
mod tests {
    ...
    fn round_trip() {
        let e = sample();
        let parsed = Entry::parse(&e.serialize()).unwrap();
        assert_eq!(parsed, e);
    }
    ...
    fn round_trip_minimal() {
        ...
        assert_eq!(Entry::parse(&e.serialize()).unwrap(), e);
    }
    ...
    fn unknown_tag_is_skipped() {
        ...
        // Append an unknown record (tag 0x7FFF) — must be ignored, entry still parses.
        tlv::write_record(&mut bytes, 0x7FFF, b"future-field");
        assert_eq!(Entry::parse(&bytes).unwrap(), sample());
    }
    ...
    fn bad_id_length_rejected() {
        ...
        assert!(Entry::parse(&bytes).is_err());
    }
}
